...
- `TodayView`: Smart dashboard showing predicted nap times, bedtime, and current status. **Empty state:** when user has no baby and has pending invite, shows "You have a baby invite" + "Review invite" CTA (navigates to Profile menu). Hero card uses `var(--bg-card)` and no border (softer than bento). Uses compact horizontal card layout (~48px height) with timeline river for mobile. **Shows skeleton loading states** via `SkeletonTimelineCard` during data fetch. Predictions shown alongside active naps; bedtime updates in real-time based on active nap's expected wake time. **Empty-state greeting is time-based** (morning/afternoon/evening; KF-13). **Prediction guards:** nap predictions are suppressed while a night sleep is active (KF-03); an overtired baby (past age max wake window) gets a gated "NAP NOW" instead of silence (KF-04); bedtime is clamped ≥ the last projected nap's end (KF-08)
- `QuickActionSheet`: Napper-style bottom sheet with quick action grid (Wake Up, Nap, Bedtime) + **second row** "Night waking" button (storm cloud icon, `--wake-color`). **Wake Up tile only shows when something is asleep/unended** (`canWakeUp` prop; otherwise a 2-tile Nap/Bedtime grid) — without it the tile dead-ended in a >14h night-sleep error (lessons §22.1, KF-01). **Night waking row** shows while a bedtime is active OR the morning after a completed night (~16h) for retrospective logging (KF-06). Uses framer-motion spring animations. Opens SleepEntrySheet with current time pre-loaded
- `SleepEntrySheet`: Bottom sheet modal for adding/editing sleep entries with time pickers. **Uses Framer Motion with drag-to-dismiss** (swipe down to close with elastic physics). Shows selected date and uses smart defaults (12:00 for naps, 20:00 for bedtime) when adding entries for past dates. **Pause section:** collapsible cards for completed pauses (add/edit/delete); contextual labels ("Pause" for naps, "Night waking" for bedtime with storm cloud icon). **Active entries:** two **labeled** buttons (Pause/"Night waking" + Stop/"End night"/"End nap") with pulsing ring when paused, timer freezes showing net sleep; status drops "Sleeping…" while a waking is open (KF-05, KF-14). **Validation** errors render in `var(--wake-color)` amber, not red (PRD §4.2; KF-09); very-short naps (<5min) get a gentle warning (KF-16). The "log wake-up" variant (`defaultEndTimeToNow`) shows a sunrise/parchment "Wake up" header, not moon/"Night sleep" (KF-10). **Qualitative section:** onset chips (multi-select: Long time to fall asleep, Fussy), sleep method chips (single-select: In bed, Nursing, etc.), notes textarea. All optional.
...
- `SubViewHeader`: Centered title + subtitle (Napper-style); back arrow absolute left. Container has `px-12` so a long subtitle can't slide under the back button on narrow screens (KF-12). Used by My Babies, Settings, Support, About, FAQs, Contact, Privacy, Terms, Baby detail.
...
- `EntryChoice`/`OnboardingFlow`: Pre-auth onboarding (Welcome → Baby name → Baby DOB → Your name → Your relationship → Account). UX/UI only; Next disabled until step complete. **DOB defaults to empty** (Next stays disabled until a date is picked — no "born today" babies; KF-02). **Account step has a back button** (`SignUpForm` `onBack` → `goBack`) so users can return to fix name/relationship (KF-07). See `product-research/onboarding/onboarding-plan.md`.
...
When critiquing or proposing design work (mockups, flows, UX, audits, prototypes), apply **`.context/guidelines/design_critique.md`**. Spine:

- Separate **fidelity** (polish) from **quality** (goal-achievement). Judge work **successful / unsuccessful relative to specific user, business & technical goals** — not "good"/"bad" — and name the goals you're assuming if they aren't stated. Replace "I like it" with a concrete next test.
- For **uncertain / AI-mediated / high-stakes** decisions only (gate it — skip trivial calls): reason probabilistically — explicit assumptions + confidence, 2–3 options framed as bets, cheap experiments / HITL checkpoints, communicate uncertainty, and flag any deterministic UI wrapping a probabilistic system.

Full rules and rationale: **`.context/guidelines/design_critique.md`**.
