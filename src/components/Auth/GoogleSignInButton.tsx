...
export function GoogleSignInButton({ onSignIn, disabled = false }: GoogleSignInButtonProps) {
  ...
  const handleClick = async () => {
    ...
    // Note: On success, the page redirects to Google, so no need to reset loading
  };
  ...
  return (
    <div className="w-full">
      ...
      {error && (
        <p className="mt-2 text-sm text-[var(--danger-color)] text-center font-display">{error}</p>
      )}
    </div>
  );
}
