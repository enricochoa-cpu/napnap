...
## Stats screen (2026-02-23)

- **Insight chip** (“Solid sleep patterns” / “Building patterns”): commented out (not useful per user feedback).
- **KPIs:** Avg. total sleep, avg. night sleep, avg. nap time, avg. naps/day (summary); avg. nap time & avg. naps/day (Naps); avg. night sleep (Night) — all commented out, not deleted, so they can be restored.
- **Naps section:** Added **Average nap** chart: bar chart of per-day average nap duration (nap minutes / nap count for that day). Uses `averageNapChartData` and `AvgNapTooltip`.
