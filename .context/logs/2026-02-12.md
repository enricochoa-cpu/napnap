...
## Typography: Plus Jakarta Sans (same day)

- **Replaced** Quicksand (display) + Nunito (body) with **Plus Jakarta Sans** for both display and body.
- **Rationale:** More professional, cohesive look; single family improves consistency. Quicksand/Nunito felt too informal.
- **Updated:** `src/index.css` (Google Fonts import, `--font-display`, `--font-body`), CLAUDE.md, design_system.md, frontend_guidelines.md, tech_stack.md, README.md, Supabase invitation email (`font-family`).
- **Landing (EntryChoice):** Order is napnap (lowercase) → moon icon → tagline → Get started / I have an account; duplicate "Already have an account? Sign in" link removed.
