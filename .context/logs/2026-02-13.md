...
- **ESSENTIAL-GAPS.md** (Priority 3 — Multi-baby): Documented that DB is not ready for 1 user + 2+ own babies. Added **To do** checklist: new `babies` table, `profiles` user-only, `sleep_entries.baby_id`, `baby_shares.baby_id`, app/RLS alignment. Updated matrix: switch/scope Done (UI); schema still missing.
