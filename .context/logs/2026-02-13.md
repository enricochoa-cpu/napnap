...
- **BACKLOG.md** (Priority 3 — Multi-baby): Documented that DB is not ready for 1 user + 2+ own babies. Added **To do** checklist: new `babies` table, `profiles` user-only, `sleep_entries.baby_id`, `baby_shares.baby_id`, app/RLS alignment. Updated matrix: switch/scope Done (UI); schema still missing.
- **progress.txt**: Multi-baby pending item updated with pointer to ESSENTIAL-GAPS Priority 3 for full DB change checklist.
...
- **BACKLOG.md**: Priority 2 (Delete account) updated to implemented.
- **plans/delete-account-with-anonymization.md**: Status note (implemented).
- **app_flow.md**: Delete account confirmation no longer TODO.
- **MEMORY.md**, **progress.txt**, **BACKLOG.md**, **lessons.md**: Onboarding persistence and first-login profile write documented; Priority 1 (persist draft + write profile on first login) marked done; new lesson §12.1 (onboarding draft not persisted).

---

---

## Sleep Report feature (same day)

- **Report window:** Last **30 days** only (not Stats date range). Rationale: babies vary a lot; parents need a precise, recent picture. `REPORT_CAP_DAYS = 30` in SleepReportView; entries filtered to that range before `getReportData()`.
- **Button copy:** "Generate report (last 30 days)" in Stats tab so expectations are explicit. Report header subtitle "Last 30 days"; Overview subtitle "Based on the last 30 days of logs."
- **Overview:** Warm tone, no dates (e.g. "This is [name]'s sleep so far. He's/She's [age] — here's what we're seeing.").
- **Sections:** Bedtime & wake times, Patterns we're seeing, What to try use **icon bullets** (moon, sun, pattern, check) and **same line height** (`BULLET_LIST_CLASS`: `space-y-2 leading-relaxed text-sm`). Bullet accent: `--night-color`.
- **Age calculation fix (dateUtils):** "X months, Y days" was wrong (e.g. 7 months, 2 days instead of 7 months, 28 days). Root cause: `days = differenceInDays(now, dob) % 30`. Fix: `days = differenceInDays(now, addMonths(dob, months))` so Y is days since last month anniversary. Documented in lessons.md §13.1.

---

## Context updates (Sleep Report + age fix)

- **sleep-report-prd.md:** Implementation note added (30-day cap, button copy, bullets, age); summary table and §7 updated where possible.
- **app_flow.md:** Stats section updated with "Generate report (last 30 days)" and report sub-view description.
- **lessons.md:** New §13 (Display / dateUtils), §13.1 baby age days calculation.
- **MEMORY.md, progress.txt, CLAUDE.md:** Sleep Report and age fix referenced.
