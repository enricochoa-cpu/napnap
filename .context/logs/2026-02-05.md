...
### 8. Baby Gallery & Edit Sheet Cleanup
- **Removed "Quick Info" section**: Redundant UI removed
- **Single edit trigger**: Owned baby cards → tap to edit (no separate Edit button)
- **Shared baby cards**: Tap to select (no edit)
- **"Tap to edit" hint**: Subtle text on owned cards

### 9. High-Fidelity Glass Sheet (BabyEditSheet)
- **Background**: `bg-[#1a1c24]/95` with `backdrop-blur-2xl`
- **Corners**: `rounded-t-[40px]` organic feel
- **Drag handle**: `w-10 h-1 bg-white/20`
- **Form fields**: Borderless inputs in soft `rounded-2xl` cards
- **Actions**: Save button, Cancel link, Delete at bottom (red text)
- **Overlay**: `bg-black/30 backdrop-blur-sm`

### Bug Fix: Invited Users Can't Save Profile
- **Issue**: Users without their own baby couldn't save name/role changes
- **Symptom**: UI showed changes, but reverted on refresh
- **Root cause**: `updateProfile()` used `.update()` which only affects existing rows
- **Fix**: Changed to `.upsert()` to create profile row if it doesn't exist
...
8. `refactor: cleanup Baby Gallery and high-fidelity Glass Sheet`
9. `fix: allow invited users without baby to save profile changes`
