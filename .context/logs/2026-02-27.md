...
## Support + Sleep Log i18n and bedtime card fix (later same day)

### Support and nested pages translated

- **SupportView:** All copy now uses i18n: title/subtitle, FAQs/Contact/Terms/Privacy menu row titles and subtitles, quick-tip card. New keys in `support.*`: `faqsMenuSubtitle`, `contactMenuSubtitle`, `termsMenuTitle`, `termsMenuSubtitle`, `privacyMenuTitle`, `privacyMenuSubtitle`, `quickTipTitle`, `quickTipBody`. Added in en, es, ca.
- **PrivacyPolicyView:** Header and footer use i18n: `privacy.viewTitle`, `privacy.viewSubtitle`, `privacy.lastUpdated`. Section body content still from `PRIVACY_POLICY_SECTIONS` constant (English) for now.
- **FAQsView, ContactView, TermsOfServiceView:** Already used `t()`; no change. Support menu and nested view headers now respect app language (en/es/ca).

### Bedtime card label (Sleep Log)

- **SleepEntry.tsx (BedtimeEntry):** Card title was `sleepEntrySheet.addBedtime` ("Add bedtime"), which was wrong when showing an existing or active night sleep. Switched to `sleepEntry.nightSleep` ("Night sleep" / "Son nocturno" / "Son nocturn") so the card describes the entry, not the action. "Add bedtime" remains only in SleepEntrySheet when creating a new night entry.

### History view header

- **App.tsx (renderHistoryView):** Title and empty-state button use i18n: `t('history.sleepLog')` and `t('nav.addBabyToStart')`.
