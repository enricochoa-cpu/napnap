...
Short, Napper-inspired onboarding (UX/UI only). Persistence and schema are out of scope for this phase. Reference: [napper-onboarding-reference.md](napper-onboarding-reference.md) (Napper analysis); we do something **similar, not the same** — shorter path, our tone and design tokens.
...
Pending onboarding work is tracked in **`.context/ESSENTIAL-GAPS.md`** (section 1): "Has completed onboarding" flag so returning users skip Entry choice and go to Login; optional persist draft across refresh.
