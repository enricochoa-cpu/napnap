...
> - **Remaining sweep — KF-06, KF-07, KF-09, KF-10, KF-12, KF-13, KF-14, KF-16** implemented; build + lint green (0 errors). Verified live: KF-07 (back button on account step → returns to relationship; shot 39), KF-09 (validation errors now amber, not red; shot 37), KF-12 (header subtitle no longer clipped; shot 38), KF-14 ("Sleeping…" gone while paused; shot 35), KF-16 (very-short-nap warning; shot 36). KF-13 (time-based greeting), KF-10 (wake-aware header — now defensive since KF-01 removed the dead-end entry point), and KF-06 (night-waking FAB the morning after a completed night) are code-verified.
> - **KF-15** — evaluative copy softened ("Upset"→"Fussy", "Bad mood"→"Grumpy"); the emoji→SVG visual swap is **deferred** (needs a cohesive ~15-icon set designed deliberately — rushing it risks regressing the aesthetic vs the recognizable emoji).
> - **KF-11** — **not changed** (deliberate): multi-owned-baby is a schema migration + product decision, not a UI sweep item. The plural "Baby profiles" title is acceptable today because the gallery legitimately mixes the owned baby with shared babies.
>
